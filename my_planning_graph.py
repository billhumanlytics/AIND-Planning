...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if new_node.prenodes.issubset(self.s_levels[level]):
                result.add(new_node)
                for node in new_node.effnodes:
                    node.parents.add(new_node)
                    new_node.children.add(node)
        self.a_levels.append(result)
    ...
    def add_literal_level(self, level):
        ...
        for node in self.a_levels[level-1]:
            effnodes = node.effnodes
            result = result.union(effnodes)
            node.children = node.children.union(effnodes)
            for snode in effnodes:
                snode.parents.add(node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for item in node_a1.action.effect_add:
            if item in node_a2.action.effect_rem:
                return True
        for item in node_a1.action.effect_rem:
            if item in node_a2.action.effect_add:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for item in node_a1.action.effect_rem:
            if item in node_a2.action.precond_pos:
                return True
        for item in node_a2.action.effect_rem:
            if item in node_a1.action.precond_pos:
                return True
        for item in node_a1.action.effect_add:
            if item in node_a2.action.precond_neg:
                return True
        for item in node_a2.action.effect_add:
            if item in node_a1.action.precond_neg:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for item in node_a1.parents:
            for item2 in node_a2.parents:
                if item.is_mutex(item2):
                    return True

        for item in node_a1.action.precond_neg:
            if item in node_a2.action.precond_pos:
                return True

        for item in node_a2.action.precond_neg:
            if item in node_a1.action.precond_pos:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        Case 1: if the states are negative of each others: At(C1, P1) and Not At(C1, P1), than they
        are mutex
        ...
        for item in node_s2.parents:
            if node_s1 in item.children:
                return False
            flag = False
            for item2 in node_s1.parents:
                if node_s2 in item2.children:
                    return False
                if not item.is_mutex(item2):
                    flag = True
            if not flag:
                return True
